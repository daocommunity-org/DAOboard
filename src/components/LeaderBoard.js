...
function LeaderBoard() {
    ...
    const { membersdata, currentUser, providerConnected, pointsToToken } = useContext(AppConfig);
            ...
            >
                <Box sx={style}>
                    <div className='flex flex-col items-center justify-center gap-4'>
                        <div className='font-semibold'>Hey {modalName}! How much tokens would you like to withdraw? </div>
                        <input className='rounded-lg p-2' onChange={e => setWithdrawTokens(e.target.value)} type="text" />
                        <button onClick={claimTokens} className='w-fit h-fit p-2 bg-slate-500 rounded-xl  border-2 border-blue-200 active:bg-slate-400 transition-all ease-in-out hover:scale-105 hover:bg-blue-300 cursor-pointer'>Withdraw Tokens</button>
                    </div>
                </Box>
            </Modal>
